...
sub(" ", "@", word)
gsub("Aa", "", word) 
gsub("Aa{2}", "", word) # {} 안에 숫자는 바로 앞에 문자를 반복
gsub("(Aa){2}", "", word) # {} 안에 숫자는 바로 ()안에 문자를 반복
gsub("[Aa]", "", word) 
...
grep("chas?e", words, value=T) # chae, chase
...
as.POSIXct("2022/01/01 08:15:00") - as.POSIXct("2022/01/01 05:00:00")
as.POSIXlt("2022/01/02 08:15:00") - as.POSIXlt("2022/01/01 05:00:00")

ct<-Sys.time()
lt<-as.POSIXlt(ct)
str(ct) 
str(lt) 
unclass(ct)  # 1970-01-01을 기준으로 얼마나 시간이 지났지는 지의 값을 가지고 있다.
unclass(lt) 
lt$mon+1 # 1월을 0으로 가지고 있어서 +1을 해줘야 정확한 월을 확인할 수 있다.
lt$hour
lt$year+1900 # 1900을 더해줘야 현재 연도에 맞게 추출 가능

?as.POSIXlt
as.POSIXct(1449894438,origin="1970-01-01")
as.POSIXlt(1449894438,origin="1970-01-01")
as.POSIXlt(1647937713,origin="1970-01-01")

as.POSIXlt("2022/05/01")$wday # 이번주에서 몇일째 되는 날이냐?
as.POSIXlt("2022/05/02")$mday # 이번 달에서 몇일째 되는 날이냐?
as.POSIXlt("2022/05/03")$yday # 이번 년에서 몇일째 되는 날이냐?
...
christmas2$wday
# 내 결혼기념일 ^^
wedding.anniversary <- as.POSIXlt(("2022-08-15"))
weekdays(wedding.anniversary)
#2023년 1월 1일 어떤 요일
