...
library(ggmap)
register_google(key ='') # key는 별도로 기입 필요
register_google(key = "") # 깃허브 공유 자료라서

lon <- 126.9221322
lat <- 37.5268831
cen <- c(lon,lat)
mk <- data.frame(lon=lon, lat=lat)
map <- get_googlemap(center=cen, maptype="roadmap",zoom=1, marker=mk) # get_googlemap 구글 맵에 특화된 함수
Sys.sleep(2)
ggmap(map)
...
Sys.sleep(2)
ggmap(map)
map <- get_googlemap(center=cen, maptype="satellite",zoom=16, marker=mk) # satellite 위성 지도
Sys.sleep(2)
ggmap(map)
map <- get_googlemap(center=cen, maptype="terrain",zoom=8, marker=mk) # terrain 지형 지도
Sys.sleep(2)
ggmap(map)
map <- get_googlemap(center=cen, maptype="terrain",zoom=12, marker=mk)
Sys.sleep(2)
ggmap(map)
map <- get_googlemap(center=cen, maptype="hybrid",zoom=16, marker=mk) # hybrid 위성 지도에 지역 정보가 표시
Sys.sleep(1)
ggmap(map)+labs(title="테스트임", x="경도", y="위도")+
  theme(plot.title=element_text(family="maple", color="blue"))

# get_map 여러 지도 서비스를 사용 source 매개변수에 원하는 지도 서비스 서버 정보 지정

map <- get_map(location=cen, maptype="toner",zoom=12, marker=mk, source="google") # toner 주로 차도 위주로 표시 
ggmap(map)
...
mk <- geocode("seoul", source = "google") # 유명한 지역은 지명 넣으면 위도 경도를 알려준다
print(mk)
cen <- c(mk$lon, mk$lat)
map <- get_googlemap(center=cen, maptype="roadmap",zoom=11, marker=mk)
ggmap(map)

mk <- geocode(enc2utf8("부산"), source = "google") # enc2utf8 한글명을 입력할 때 필요한 함수
cen <- c(mk$lon, mk$lat) # center는 벡터 형식이여야 함
map <- get_googlemap(center=cen, maptype="roadmap",zoom=11, marker=mk)
ggmap(map)
...
ggmap(map) + 
  ...
  geom_text(aes(x=mk$lon, y=mk$lat, label="우리가 공부하는 곳", 
                family="maple", vjust=-0.1, hjust=-0.1), colour="blue") # vjust, hjust 마킹한 위치에서 이동하여 표시
...
# 지도 배경에 타일깔기
leaflet() %>% addTiles()  # addTiles() 함수 필수
...
map3<-leaflet() %>% addTiles() %>%  addPopups(126.97797, 37.56654, content1) # addPopups 경도 위도 정보, 팝업 내용으로 바로 지도에서 표시
map3
...
leaflet(wifi_data) %>% 
  setView(lng = seoul_lonlat[1], 
          ...
          zoom = 11) %>% 
  addTiles() %>% 
  addCircles(lng = ~lon, lat = ~lat) # ~ 표물라 형식으로 지정해줘야 함
...
leaflet() %>%
  ...
  addProviderTiles("NASAGIBS.ViirsEarthAtNight2012") -> result

htmltools::save_html(result, "output/leaflet.html")
