...
pie(성적$국어, clockwise=T, labels=paste(성적$성명, "-", 성적$국어), 
      col=rainbow(10)) # clocwise = T는 12시가 시작, 시계방향으로 진행, defalt = 3시 시작, 반시계 방향
...
boxplot(성적2, col=rainbow(3), ylab="성적")
boxplot(성적2); str(성적2)
...
# old <- par(family = "maple") # 전역적으로 폰트를 설정
