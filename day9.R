...
webElem <- remDr$findElement(using = "css", "[name = 'q']") # css로 속성=속성값을 설정
webElem <- remDr$findElement(using = "css selector", ".gLFyf") # css 선택자로 class 값을 설정
webElem$sendKeysToElement(list("PYTHON", key = "enter"))

remDr$navigate("http://www.naver.com/")
Sys.sleep(1) # 네트워크 등 여러 이유로 웹페이지 이동시 지연 시간을 줘서 안정적인 크롤링을 한다.
...
one<-remDr$findElement(using='css selector', 'div') # 노드 한 개 리턴(WebElement 객체)_첫번째 하나
...
one$getElementTagName()
one$getElementText()
# 클릭 이벤트 처리하는 또 다른 방법, arguments[0].click() 자바스크립트 코드
# 자바스크립트 코드로 강제로 클릭 이벤트를 발생! 가끔씩 clickElment()가 에러날 때 사용 가능
