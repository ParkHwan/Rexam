...
str(iris)
View(iris)
iris %>% pull(Species) #pull은 무조건 vector로 출력
iris %>% pull(Sepal.Length)
iris %>% pull(Sepal.Length, Species) # 뒤에 있는 변수가 이름이 되고 앞에 변수가 값이 되서 namedvector가 된다.
iris %>% pull(-1) # 제일 마지막을 출력
iris %>% pull(-5) # 제일 앞에거를 출력
iris %>% pull() # 기본값 -1
iris %>% select(Species) #select는 무조건 dataframe로 출력
...
iris %>% select(starts_with("petal")) %>% head(1) # 변수명 대소문자를 구분하지 않는다.
iris %>% select(starts_with("petal", ignore.case=F)) %>% head(1) #ignore.case=F -> 대소문자를 구분하는 매개변수

# Select column whose name ends with "Width"
iris %>% select(ends_with("Width")) %>% head(1)

# Select columns whose names contains "etal"
iris %>% select(contains("etal")) %>% head(1) # contains() 변수를 포함하는 값을 출력
...
# one_of -> 찾고자 하는 변수가 없어도 워닝 에러 발생하고 출력 가능한 변수만 출력
...
exam %>% arrange(desc(math), desc(english), desc(science)) %>% head(3)
...
iris %>% select(Species)
iris %>% distinct(Species) # unique 함수와 동일, 중복된 행은 생략된다.
...
fill(exam, math, .direction = "down") # 바로 위의 데이터를 내려서 NA를 대체
fill(exam, math, .direction = "up") # 바로 아래의 데이터를 올려서 NA를 대체
fill(exam, math, .direction = "updown") # 데이터를 up으로 채우고 채워지는 않는 부분은 down해서 NA 대체
