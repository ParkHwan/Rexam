...
sapply(flower, function(d) {y <- paste("-",d,"-", sep=""); return(y)})
...
# 목록 아이템 추출
title <- html_text(html_nodes(html, "#apiDataList span.title"), trim = TRUE)
title
...
sports.baseball <- read_html(url); crowdworks

# 참여 가능한 작업 목록 추출
projectTitle <-  html_text(html_nodes(crowdworks, "#workingPrj strong.projectName"))
projectTitle
